...
echo "3 - importing assets into warehouse report db ..."
...
echo "4 - importing connections into warehouse report db ..."
...
echo "5 - attempting to remove duplicates ..."

set_agg='{"$project":{"*":1,"__federation_source":0}},{"$group":{"_id":"$_id","set":{"$addToSet":"$$ROOT"}}},{"$project":{"*":1,"count":{"$size":"$set"},"pick":{"$cond":{"if":{"$eq":[{"$size":"$set"},1]},"then":{"$arrayElemAt":["$set",0]},"else":"conflict"}}}}'
...
echo "6 - creating asset/connection overview report ..."
