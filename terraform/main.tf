...
data "template_cloudinit_config" "sonar_config" {
  ...
  part {
    ...
    content  = <<-END
        ...
        curl -k https://${var.s3_bucket}.s3.${var.region}.amazonaws.com/${var.sonar_config_file} -o /tmp/${var.sonar_config_file}
        
        sudo tar -xzvf /tmp/${var.sonar_install_file} -C /opt
        
        sudo /opt/jsonar/apps/${var.sonar_version}/bin/python3 /opt/jsonar/apps/${var.sonar_version}/bin/sonarg-setup install-from-encrypted-file /tmp/${var.sonar_config_file} ${var.sonar_config_file_password} --accept-eula 
    END
  }
}
